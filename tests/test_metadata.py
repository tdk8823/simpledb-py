...
from simpledbpy.metadata import TableManager
...
class TestRecordPage(unittest.TestCase):
    ...
    def test_basic_usage(self) -> None:
        ...
        table_manager = TableManager(is_new=True, tx=tx)

        schema = Schema()
        schema.add_int_field("A")
        schema.add_string_field("B", 9)
        table_manager.create_table("MyTable", schema, tx)

        layout = table_manager.get_layout("MyTable", tx)
        schema_from_table_manager = layout.schema
        self.assertEqual(schema.fields, schema_from_table_manager.fields)
        self.assertEqual(
            layout.slot_size, 4 + 4 + 4 + 9
        )  # 4 bytes for flag, 4 bytes for int, 4 bytes for string length, 9 bytes for string
        for field in schema_from_table_manager.fields:
            if field == "A":
                self.assertEqual(Types.INTEGER, schema_from_table_manager.type(field))
            elif field == "B":
                self.assertEqual(Types.VARCHAR, schema_from_table_manager.type(field))
                self.assertEqual(9, schema_from_table_manager.length(field))
            else:
